...
if [ "$EUID" -ne 0 ]; then
    ...
    show_progress "Compilando, isso pode levar bastante tempo dependendo da maquina..."
    mkdir -p /opt/rustymanager
    mkdir -p /opt/rustymanager/ssl
    git clone --branch "$SCRIPT_VERSION" --recurse-submodules --single-branch https://github.com/vmell0/RustyManager.git /root/RustyManager > /dev/null 2>&1 || error_exit "Falha ao clonar RustyManager"
